...
Live at **https://monopoline.vortal.space**. Part of [Vortal](https://vortal.space).

## Run and deploy

Monopoline runs on Cloudflare Workers. `public/` is served as static assets from Cloudflare's edge, and
each table lives in its own Durable Object (`cloudflare/worker.js`), so tables survive deploys and
restarts. It fits the Workers free plan, and nothing sleeps. From this folder:

```bash
npx wrangler dev      # local, at http://localhost:8787
npx wrangler deploy   # live at monopoline.vortal.space
```

The first deploy creates the Worker, both Durable Object classes, and the `monopoline.vortal.space`
custom domain with its DNS record and certificate.

- **Live updates** use WebSockets, falling back to Server-Sent Events when a network blocks sockets.
- **Rooms** are saved per table and cleared 8 hours after their last activity. Pass-and-play games are
  saved in the browser.
- **Bug reports** land in Workers Logs (**Workers & Pages → monopoline → Logs**).
- **Moderator key:** `npx wrangler secret put ADMIN_KEY`.
- **Look:** the Vortal palette, in dark, is the default. Players can pick another palette or Light /
  Follow device under **Theme & palette**.
...
```
wrangler.jsonc            Cloudflare config: static assets, Durable Objects, custom domain
cloudflare/worker.js      the server: room API, WebSockets (SSE fallback), Room + Hub Durable Objects
...
| `GET` | `/api/rooms/:code/socket` | WebSocket: `sync`, `state`, `roster`, `say`, `signal`, `voice` |
| `GET` | `/api/rooms/:code/events` | the same stream as Server-Sent Events, for networks that block sockets |
...
snapshot (positions, cash, deeds, last 25 log lines). Every report is logged, so it shows up in
Workers Logs (**Workers & Pages → monopoline → Logs**):

```
[bug] 2026-08-17T01:18:41.914Z | Ada @7S4U | End turn did nothing after doubles
```

To read them back over HTTP instead, set an `ADMIN_KEY` secret (`npx wrangler secret put ADMIN_KEY`) and
fetch `/api/reports?key=YOUR_KEY`. Without that secret the endpoint stays off. The latest 200 reports are
kept in the server's storage. Offline players get a **Copy report** button instead, so nothing is lost
...
audio never touches it. Needs HTTPS (Cloudflare provides it) and microphone permission. Lightweight mode
turns it off.
...
since without it anyone who finds the endpoint can use them. Set it with
`npx wrangler secret put ADMIN_KEY` and enter the same value in the console.
